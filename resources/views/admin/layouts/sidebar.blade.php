<aside 
    ...
    <!-- SIDEBAR HEADER -->
    <div :class="sidebarToggle ? 'justify-center' : 'justify-between'" class="flex items-center gap-2 pt-4 pb-7 sidebar-header">
        <a href="{{ route('admin_dashboard') }}">
            <span class="logo" :class="sidebarToggle ? 'hidden' : ''">
                <img src="{{ asset('logo/logo.png') }}" alt="Logo" class="h-12" />
            </span>
            <img class="logo-icon" :class="sidebarToggle ? 'lg:block' : 'hidden'" src="{{ asset('logo/logo.png') }}" alt="Logo" />
        </a>
    </div>
...
<li x-data="{ open: {{ $page === 'profile' ? 'true' : 'false' }} }" >
    <button @click="open = !open" 
        ...
        <i class="fas fa-chevron-down ml-auto" :class="open ? 'rotate-180 transition-transform' : 'transition-transform'"></i>
    </button>
